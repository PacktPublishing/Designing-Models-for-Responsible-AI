...
# 
# In this example we develop a secure MPC protocol for the [Secret Santa](https://en.wikipedia.org/wiki/Secret_Santa) problem.
# Traditionally, a group of family and friends gathers to put all their names in a hat and then randomly draw names from the hat.

# Mathematically, the Secret Santa problem is about generating so-called [derangements](https://en.wikipedia.org/wiki/Derangement),
# which are permutations without fixed points. A permutation is a one-to-one mapping on a set of numbers, and a fixed-point is a number that is mapped to itself.
# 
# We present an MPyC program (a Python program using the `mpyc` package) for generating uniformly random derangements. 
# In this example, the MPyC program is run by a single party only. However, this very same MPyC program can be run between multiple parties to
#  generate secret-shared random derangements, as will be shown at the end. These random derangements will remain secret *forever* unless a majority of these parties collude.
# 
# ## MPyC setup
# 
