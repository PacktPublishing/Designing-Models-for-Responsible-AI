...
In this example we learn about Secure multiparty computation (MPC / SMPC) , a cryptographic protocol that distributes a
computation across multiple parties where no individual party can see the other parties’ data. Here we learn how to train
an efficient secured Ridge Regression model. This problem exploits the MPC algorithm by using a
MPyC framework, that is based on threshold secret sharing (MPC based on Shamir secret sharing) over finite fields.

MPyC demo shown here is from the paper 'Efficient Secure Ridge Regression from
