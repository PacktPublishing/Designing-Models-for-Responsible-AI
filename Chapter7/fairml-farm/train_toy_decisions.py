...
In this example we learn about how to use linear classifiers make fair decisions on toy data, generated synthetically.
In this example below we see how the decision boundary shifts as we change the hyperparameter
weighting classification loss vs. demographic parity fairness loss for a dataset having 2 classes.
We use the default dpe_scalar (demographic parity error ) option.

You can go through the results folder. here we see the two demographic groups are red and blue,
with different local concentrations. Green is a positive sample, while magenta is negative.
We can further observe the behavior for different fairness losses, or change the generated distribution being learned.
