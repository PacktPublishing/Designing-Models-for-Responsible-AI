'''
In this example, we use the concept put forward by Natalia Martinez, Martin Bertran, and Guillermo Sapiro
on minmax fairness criteria. This minmax crieria aims to improve group fairness metrics.
Let us now study an important concept of minmax fairness (https://github.com/amazon-research/minimax-fair)
that strives to achieve fairness when protected group labels are not available.

We have changed some of hyper-parameters from that what is present in github
https://github.com/amazon-science/minimax-fair/blob/main/main_driver.py
'''
...
numsteps = 5000  # number of steps for learning/game, changed to 5000 from 2000
# NOTE: eta = a * t^(-b) on the t-th round of the game
a = 1  # Multiplicative coefficient on parametrized learning rate
b = 1 / 3  # Negative exponent on parameterized learning rate, changed to 1/3 from 1/2
...
tol = 1e-12  # The tolerance on the gradient for logistic regression convergence, sklearn default is 1-e4, changed to 1e-12 from 1e-15
...
C = 1e12  # Inverse of regularization strength, ignored when penalty = 'none'. Set to 1e15 to simulate no regularization, for the current run,
#changed to 1e12 from 1e15
...
n_epochs = 2500  #changed to 2500 from 2000
lr = 0.5         # changed to .5 from .1
momentum = 0.75 #changed to .75 from 0.9
weight_decay = 0
...
numdims = 12  # dimensionality of synthetic data
